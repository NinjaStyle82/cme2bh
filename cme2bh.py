...
parser.add_argument('-n','--netbios', help='Credentials for Neo4j: user:pass', required=True)
parser.add_argument('-p','--path',  default="./cme.db", help='Path to CME DB file (Default: "./cme.db")')

args = parser.parse_args()

auth = base64.b64encode(args.creds.encode())

# if there is a link in hosts > links then we are admin
...
c = conn.cursor()
# for host with a link in links database (host nas an account with admin), mark as owned
